...
def nodes_edges_to_gdfs(Gs, crs_epsg=4326):
    ...
    # Preserve length in meters before reprojecting geometry
    ...
    # Restore metric lengths (geometry is now in degrees, but length should stay in meters)
    if 'length_m' in edges.columns:
        edges['length'] = edges['length_m']
        edges = edges.drop(columns=['length_m'])
