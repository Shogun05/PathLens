...
class Sat2GraphDataLoader():
	...
	def loadtile(self, ind):
		...
		try:
			sat_img = cv2.cvtColor(cv2.imread(self.folder + "/region_%d_sat.png" % ind), cv2.COLOR_BGR2RGB).astype(np.float)
		except:
			sat_img = cv2.cvtColor(cv2.imread(self.folder + "/region_%d_sat.jpg" % ind), cv2.COLOR_BGR2RGB).astype(np.float)
